...
def test_user(host):
    ...
    assert u.exists
    assert u.uid == 2222
    assert set(u.groups) == set(["ansibletestuser", "users", "bin"])
    assert u.shell == "/bin/sh"
    assert u.home == "/home/otherdirectory"


def test_profile(host):
    f = host.file("/home/otherdirectory/.profile")
    assert b"alias ll='ls -lah'" in f.content
    assert b"alias cp='cp -iv'" in f.content


def test_public_key(host):
    cmd = "ssh-keygen -l -f /home/otherdirectory/.ssh/authorized_keys"
